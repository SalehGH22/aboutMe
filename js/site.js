/** Notice: This file contains works from many authors under various (but compatible) licenses. Please visit http://www.computerhope.com/notices for more information. */
...
$(document).ready(function(){selectnav('nav',{nested:false,label:false});$("footer input[type='text']").Watermark("Ask another computer question","#bbb");});
...
function showonlyone(thechosenone) {
      var requirements = document.getElementsByTagName("div");
            for(var x=0; x<requirements.length; x++) {
                  name = requirements[x].getAttribute("class");
                  if (name == 'requirements') {
                        if (requirements[x].id == thechosenone) {
                        requirements[x].style.display = 'block';
                  }
                  else {
                        requirements[x].style.display = 'none';
                  }
            }
      }
}
